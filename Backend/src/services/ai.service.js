...
function isTemporaryAiError(error) {
    const status = error?.status || error?.error?.code || error?.cause?.status
...
async function generateContentWithRetry(request, maxAttempts = 3) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
            ...
            if (!isTemporaryAiError(error) || attempt === maxAttempts) {
                throw error
            }

            // Brief backoff for temporary provider overload/rate-limit responses.
